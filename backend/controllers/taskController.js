...
const updateTaskChecklist = async (req, res) => {
    try{
        ...
        if (!task.assignedTo.includes(req.user._id) && req.user.role !== "admin") {
            return res
                .status(403)
                .json({message: "Not authorized to update this task checklist"});
        }
...
const getDashboardData = async (req, res) => {
    try{
        ...
        const taskByPriorityRaw = await Task.aggregate([
            {
                $group: {
                    _id: "$priority",
                    count: { $sum: 1 }
        ...
        const taskPriorityLevels = taskPriorities.reduce((acc, priority) => {
            acc[priority] = 
                taskByPriorityRaw.find(item => item._id === priority)?.count || 0;
            return acc;
        }, {})
